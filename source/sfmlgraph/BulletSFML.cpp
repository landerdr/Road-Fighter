...
BulletSFML::BulletSFML(const std::shared_ptr<sf::RenderWindow>& window, float x, float y) : window(window)
{
        ...
        r_w = static_cast<float>(shape.getSize().x * roadfighter::Transformation::Instance()->getScaleX() * 4.0 /
                                 window->getSize().x);
        r_h = static_cast<float>(shape.getSize().y * roadfighter::Transformation::Instance()->getScaleY() * 3.0 /
                                 window->getSize().y);
        c_x = x;
        c_y = y;
}
